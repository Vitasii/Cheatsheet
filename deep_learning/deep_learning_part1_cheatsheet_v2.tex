...
\key{Backprop}: for $y=f(x)$ with Jacobian $J=\partial y/\partial x$, chain rule gives $\nabla_xL=J^\top\nabla_yL$. Define $\delta^{(\ell)}=\partial L/\partial z^{(\ell)}$:
\[
...
\]
\key{Activation}: $\tanh(x)=\frac{e^{2x}-1}{e^{2x}+1}$; sigmoid/tanh saturate; ReLU $\max(0,z)$ keeps positive-side gradient; ELU/smooth variants improve negative side. \lowterm{Subgradient}: ReLU at $0$ uses any value in $[0,1]$ by convention.
...
\key{GD/SGD}: full-batch GD uses all data gradient; SGD uses one sample noisy gradient; mini-batch uses $B$ samples to trade variance vs. throughput. $x_{k+1}=x_k-\eta_k\nabla f(x_k)$. $L$-smooth means gradient Lipschitz:
$\|\nabla f(x)-\nabla f(y)\|\le L\|x-y\|$; if twice differentiable, $\|\nabla^2f(x)\|_{\rm op}\le L$ (convex: $0\preceq\nabla^2f(x)\preceq LI$). Then
\[
f(y)\le f(x)+\nabla f(x)^\top(y-x)+\frac L2\|y-x\|^2,
\]
\lowterm{Strong convexity}: $f(y)\ge f(x)+\nabla f(x)^\top(y-x)+\frac\mu2\|y-x\|^2$; with $L$-smooth and twice differentiable, $\mu I\preceq\nabla^2f(x)\preceq LI$. GD has linear convergence with $k=O(\kappa\log(1/\epsilon))$, $\kappa=L/\mu$. Thus $0<\eta<2/L$ decreases the smooth upper bound. \key{Newton}: from quadratic Taylor,
\[
x_{k+1}=x_k-\eta[\nabla^2f(x_k)]^{-1}\nabla f(x_k).
\]
...
\lowterm{Dropout}: randomly zero units, prevents co-adaptation/reliance on particular neurons or weights, approximates ensemble/implicit regularization (often L2-like). Scaling: inverted dropout divides by keep prob $\alpha$ at train and inference unchanged; standard dropout leaves train unchanged and multiplies activations by $\alpha$ at inference. \lowterm{Early stopping}: monitor held-out validation; stop when training improves but validation worsens. \lowterm{Weight decay}: L2 regularization stabilizes weights,
...
\key{BN inference}: no mini-batch training distribution; use running stats
...
\]
Asynchronous flip changes energy by $\Delta E=2y_ih_i$; if $y_ih_i<0$, energy decreases. Finite states imply convergence to a local minimum. \key{Hebbian storage}: for $K$ patterns, $w_{ij}=\frac1K\sum_{p=1}^K y_i^py_j^p$, $w_{ii}=0$; stability sufficient condition $K\le N/(4\log N)$ for random patterns. Multiple patterns cause \lowterm{spurious minima}.
\key{Optimization view}: make desired patterns valleys and undesired states high energy. For $E(y)=-\frac12y^\top Wy$, objective $\min_W\sum_{y\in P}E(y)-\sum_{y'\notin P}E(y')$ gives contrastive update
$W\leftarrow W+\eta(\E_{y\in P}[yy^\top]-\E_{y'}[y'y'^\top])$; cannot enumerate exponential state space, so sample $y\in P$, initialize $y'$ from desired pattern, run $2$--$4$ dynamics steps, update with $yy^\top-y'y'^\top$.

\subsection{Boltzmann Machine}
\key{Stochastic Hopfield}: $p(y)\propto e^{-E(y)}$. Conditional Gibbs update for $\{-1,1\}$ units:
\[
h_i=\sum_jw_{ij}y_j+b_i,\quad
\log\frac{P(y_i=1|y_{\neg i})}{P(y_i=-1|y_{\neg i})}=2h_i,\quad P(y_i=1|y_{\neg i})=\sigma(2h_i).
\]
...
\key{Annealing}: $p_T(y)\propto e^{-E(y)/T}$, sample $y_i(T)\sim\operatorname{Bernoulli}(\sigma(z_i(T)/T))$, then $T\leftarrow\alpha T$; lower $T$ shifts exploration to exploitation. \key{BM MLE}: for $E(y)=-\frac12y^\top Wy$,
\[
...
\[
\begin{aligned}
\nabla_{w_{ij}}L&=\frac1{N_P}\sum_{y\in P}y_iy_j-\E_{p_W}[y_iy_j]\\
&\approx\frac1{N_P}\sum_{y\in P}y_iy_j-\frac1M\sum_{y'\in S}y'_iy'_j,\quad
w_{ij}\leftarrow w_{ij}+\eta\nabla_{w_{ij}}L.
\end{aligned}
\]
Learning = \lowterm{positive phase} data correlation minus \lowterm{negative phase} model correlation. With hidden $h$, condition on visible data $v$ and average over $h\mid v$:
\[
\nabla_{w_{ij}}L(W)=\frac1{|P|}\sum_{v\in P}\E_{h\mid v}[y_iy_j]-\E_{y'\sim P_W}[y'_iy'_j],\quad
w_{ij}\leftarrow w_{ij}+\eta\nabla_{w_{ij}}L.
\]

\subsection{RBM and Contrastive Divergence}
\lowterm{RBM}: visible $v$ and hidden $h$ form a bipartite graph; no within-layer edges:
\[
E(v,h)=-v^\top Wh-b^\top v-c^\top h.
\]
Layer-wise Gibbs updates factorize:
\[
\begin{aligned}
...
\end{aligned}
\]
\key{Iterative sampling}: alternate $h\sim P(h|v)$ and $v\sim P(v|h)$; after mixing, $(v^{(\infty)},h^{(\infty)})\sim P_W$. With $K$ hidden samples per data and $M$ free samples:
\[
\widehat{\nabla}_{W_{ij}}L(W)=
\frac1{N_PK}\sum_{v\in P}\sum_{k=1}^{K}v_i^{(0)}h_{j,k}^{(0)}
-\frac1M\sum_{m=1}^{M}v_{i,m}^{(\infty)}h_{j,m}^{(\infty)}.
\]
\key{CD-1} replaces the long negative chain by one reconstruction:
\[
...
\key{MCMC}: Markov chain with stationary $\pi$. Detailed balance $\pi(x)T(x\to x')=\pi(x')T(x'\to x)$ implies stationarity. Uniform minorization
\[
\min_{z,z':\,\pi(z')>0}\frac{T(z\to z')}{\pi(z')}=\delta>0
\]
is a strong sufficient condition for uniform ergodicity. \key{MH}: proposal $q(x'|x)$, accept
\[
\alpha=\min\left(1,\frac{\pi(x')q(x|x')}{\pi(x)q(x'|x)}\right).
\]
For EBM, partition $Z$ cancels. \key{Gibbs}: update one conditional exactly; as MH, use $q(s_i\to s_i')=p(s_i'\mid s_{j\ne i})$, so accept prob. $=1$. \key{SGLD}: $x_{t+1}=x_t-\frac{\eta}{2}\nabla_xE_\theta(x_t)+\sqrt{\eta}\xi_t$.
...
\lowterm{Mode collapse}: generator covers few modes. \key{Sample diversity}: for multimodal $p$, forward KL is mode-covering, while reverse KL and JSD can be mode-seeking/collapse:
\[
\begin{aligned}
...
\end{aligned}
\]
\lowterm{Training instability}: moving target game can oscillate; if $D$'s task is too simple/strong, $G$ gets vanishing or weak gradients. \key{Inception Score} uses classifier $f(y|x)$:
\[
{\rm IS}=\exp\!\left(\E_{x\sim G}\KL(f(y|x)\|p_f(y))\right),\quad
p_f(y)=\E_{x\sim G}[f(y|x)].
\]
Since $\E_x\KL(f(\cdot|x)\|p_f)=-\E_xH(f(\cdot|x))+H(p_f)$, high IS needs confident images (low conditional entropy) and diverse classes (high marginal entropy). IS never uses $p_{\rm data}$, so it cannot detect memorization and is blind to intra-class artifacts. \key{FID}: use Inception-v3 pool3 2048-d features; estimate Gaussian statistics $(\mu_p,\Sigma_p)$ for $p_{\rm data}$ and $(\mu_G,\Sigma_G)$ for $p_G$:
\[
...
\]
\lowterm{Lipschitz constraint}: weight clipping, spectral norm, or WGAN-GP. For WGAN-GP, sample $\tilde x=(1-\epsilon)x+\epsilon\hat x$, $\epsilon\sim U(0,1)$, between real $x$ and fake $\hat x$:
\[
\max_D\ \E_{x\sim p_{\rm data}}D(x)-\E_{\hat x\sim p_G}D(\hat x)
